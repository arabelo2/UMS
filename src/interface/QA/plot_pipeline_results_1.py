...
class FieldMapPlot(BasePlot):
    ...
    def _create_plot(self) -> None:
        ...
        envelope_norm = np.flip(envelope_norm, 1)
...
class ProfileComparisonPlot(BasePlot):
    ...
    def _create_plot(self) -> None:
        ...
        tfm_lateral = self.dataset.tfm_data.axial_profile()
        tfm_axial = self.dataset.get_tfm_lateral_profile()
...
class OnePageSummaryPlot(BasePlot):
    ...
    def _plot_axial_profiles_with_original_logic(self, ax: plt.Axes) -> None:
        ...
        dt_axial_norm = self.dataset.dt_data.axial_profile() / global_max_dt
        tfm_axial_norm = self.dataset.get_tfm_lateral_profile() / global_max_tfm
    ...
    def _plot_lateral_profiles_with_original_logic(self, ax: plt.Axes) -> None:
        ...
        tfm_lateral_norm = self.dataset.tfm_data.axial_profile() / global_max_tfm
        tfm_lateral_norm = np.flip(tfm_lateral_norm)
...
class FarFieldDirectivityPlot(BasePlot):
    ...
    def _create_plot(self) -> None:
        ...
        if self.dataset.tfm_data and hasattr(self.dataset, 'tfm_peak_z'):
            ...
            tfm_profile = self.dataset.tfm_data.axial_profile()
