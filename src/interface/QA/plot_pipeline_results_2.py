...
FIXED: TFM data is now correctly transposed for pcolormesh to support non‑square matrices.
"""
...
class FieldMapPlot(BasePlot):
    ...
    def _create_plot(self) -> None:
        ...
        # 3. TFM Reconstruction (Full) - FIXED: Transpose to match (z,x) for pcolormesh
        ax3 = plt.subplot(2, 3, 4)
        envelope_norm = 20 * np.log10(self.dataset.tfm_data.normalized_matrix() + 1e-6)
        # Transpose so that rows = depth, columns = lateral
        ...
        # 4. TFM (Zoom) - with corrected slicing (envelope_norm already transposed)
        ...
        # 5. Difference between DT and TFM - both now in (z,x) orientation
        ax5 = plt.subplot(2, 3, 6)
        # envelope_norm already transposed, p_db is (z,x)
...
class ProfileComparisonPlot(BasePlot):
    ...
    def _create_plot(self) -> None:
        ...
        dt_lateral_norm = np.flip(dt_lateral_norm)      
...
class FarFieldDirectivityPlot(BasePlot):
    ...
    def _create_plot(self) -> None:
        ...
        if self.dataset.dt_data and self.dataset.dt_data.z_vals.size > 0:
            ...
            dt_profile_norm = np.flip(dt_profile_norm)
