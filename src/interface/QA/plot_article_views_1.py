...
def plot_publication_figure(root_dir):
    ...
    # --- Flip the lateral profile for publication-style display ---
    # (This makes the left side of the plot correspond to negative x)
    profile_lateral_db = np.flip(profile_lateral_db)

    # Calculate Metrics using the flipped lateral profile
    ...
    # Flip the entire 2D field horizontally to match the lateral profile orientation
    p_db = np.flip(p_db, axis=1)
    ...
    # Add crosshairs at peak (negate x because field is flipped)
    ...
    # Show Beam Width (using boundaries from flipped profile)
